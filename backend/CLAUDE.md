...
- **R2** buckets: `MANTTIO_REPORTS` (report images + generated PDFs, public reads via `CDN_BASE_URL`) and `MANTTIO_LOGOS` (`manttio-logos` — brand images uploaded via `POST /upload/logo` + the generated PWA icon set, public reads via `LOGOS_CDN_BASE_URL`). Brand assets keep their own lifecycle apart from report data.
