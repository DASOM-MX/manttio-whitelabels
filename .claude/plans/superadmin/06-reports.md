...
> calendar (17/18/12). So **reports never require a service order**: `serviceOrderId` /
...
> operations suite is enabled. Nothing in 06 may hard-depend on 18. 06 depends only on
> 02 (never on 17/18).
...
- ~~Status enum~~ — **confirmed 2026-07-06** against
  ...
  `serviceId?` — semantics owned by `18-service-orders.md` §2. **These are additive:
  the standalone reporting suite ignores `pending`/`cancelled` and leaves the order
  columns null (standalone-suite rule above) — nothing here requires 18.**
