...
| `DELETE /portal/service-requests/:id` | `cancel_service_requests` | 06 §3. Added 2026-09-03. The customer withdraws a request: terminal, reason required in the body, and it **soft-deletes** the row **and every quotation issued against it** (`delete_comment = "cancelled by client: <reason>"`), in one transaction. Refuses with **409 `request_has_service_order`** when any of those quotations already produced a **live** service order — cancelling that is staff's, and the message names the folios. `DELETE /:id` + reason is the house spelling for an audited soft delete (quotations, users, customers); `POST /:id/<verb>` stays for lifecycle moves that do not remove the row. Grant-gated, not `isAdmin`-gated, so a missing grant is the middleware's **404** — see 06 §3's open note on whether it should be a 403 like close. |
