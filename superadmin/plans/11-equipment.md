...
> **Status:** not-started · **Depends on:** 06 (CP-1); hooks into 04, 09
...
Equipment {
  ...
  materialUnitId?,              // WMS hook: the serialized unit this asset came from (09)
...
- **WMS hook (ask to 09 + backend):** when an install report consumes a *serialized*
...
- `equipment/pages/equipment-view/` — detail card (all fields, WMS unit link when
  ...
  04's report view) with the retro-link action ("attach report" — searchable report
...
- **Customer-view equipment card** (mounted in a new reserved slot in 06's customer-view,
  next to CRM/Bills): compact list of the client's units (name, kind, status, last
  service) + "add equipment" → the dialog. Coordinate the slot with 06 (record as ask).
...
- [ ] Customer-view equipment card in 06's slot (coordinate with 06)
...
### CP-3 — Polish
- [ ] WMS unit link rendered when `materialUnitId` present (link to 09's views)
...
## Open decisions / asks
- Ask to 06: reserve an **Equipment** slot on customer-view (like CRM/Bills).
- Ask to 09 + backend: serialized-unit-consumed-on-report → equipment auto-create hook
...
- Whether equipment should surface in 04's report-view (badge/links on linked reports) —
  coordinate with 04 when both land.
