...
- `GET /customers/:id/interactions?page&limit&refKind&refId` → paged, newest-first.
  ...
  and it is **rejected 400 without a `refKind`** (an id alone would filter across kinds).
  **The feed is deliberately unscoped** (owner 2026-08-22): any authenticated user reads the
  whole timeline, entries linking to role-restricted entities included — an audit trail
  filtered per viewer is not one. Access control lives on the linked entity (13 §4), never
  on the record that it happened.
