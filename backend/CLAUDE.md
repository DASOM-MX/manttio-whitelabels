...
- **Quotations (20):** `modules/quotations/` — the sales entry point. A quote freezes its catalog snapshots (`serviceName`/`unitPrice`/`uom`/`taxRate`) at creation, so repricing a service never rewrites a quote already sent; totals are computed per line (IVA differs per line) and never stored. Sending mints a per-recipient token (`report_emails` model) and mails a link; client contacts approve/decline through the JWT-whitelisted `/public/quotations/{token}`, responses are **mutable**, and the status is a pure function of the reviewer tally. `quotation_events` is append-only. The module also owns **`GET /customers/:id/quotations`** (`customer-quotations.controller.ts`), mounted onto the customers path in `index.ts` rather than living in the customers controller — quotations already imports customers for the recipient check, so the reverse would make the two circular. **`POST /quotations/:id/order` is deferred to 19** — it must open a `service_order` in one transaction and that table doesn't exist yet, so `order_created` is currently unreachable and `quotations.service_order_id` carries no FK.
