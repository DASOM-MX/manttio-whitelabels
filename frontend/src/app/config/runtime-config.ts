 ...
 *  purpose: every consumer reads it lazily (see the `base` getter in
 *  `RemoteService`), so there is nothing to notify — by the time any request
 *  is built, `loadRuntimeConfig()` has already resolved. */
 ...
 *
 *  Never rejects. Every rung failing is a supported state, and in this app it
 *  is a *routine* one: a technician on a rooftop reloads the installed PWA with
 *  no signal, the fetch fails, and the persisted value from the last online
 *  boot is what keeps the queue pointed at the right host. The compiled literal
 *  is the last rung, and it is also what makes `ng serve` work unchanged —
 *  there is no Worker under the dev server, so `environment.development.ts`
 *  wins exactly as it does today. */
export async function loadRuntimeConfig(): Promise<void> {
  try {
    const res = await fetch(CONFIG_ENDPOINT, {
      // A hung request must not hold the app hostage; see 25 §3. This matters
      // more here than in the admin: "one bar of signal" is a slow connection,
      // not a dead one, so the request can hang rather than fail fast.
