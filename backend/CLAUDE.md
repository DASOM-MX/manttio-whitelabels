...
- **Postgres** on **Neon** via `@neondatabase/serverless`'s **WebSocket driver** (not neon-http) — chosen for real transactions (the create-report flow updates a counter + a header row + N detail rows atomically; the clients module writes a customer + its contacts + fiscal atomically). Live DB is current through migration `0014` (clients CRM/fiscal/contacts extension).
...
- **Audited soft-delete on users:** `DELETE /users/:id` requires a Zod-validated `{ deleteComment }` body and stamps `users.delete_comment` + `users.deleted_by` (self-FK to `users.id`, `ON DELETE RESTRICT`) alongside `deleted_at`. The route guards self-delete (`me.id === id → 400 cannot_delete_self`). **Customers** carry the same audit columns (`delete_comment` + `deleted_by` FK to `users.id`, `ON DELETE RESTRICT`) but the comment is **optional** on `DELETE /customers/:id` (the main frontend deletes with no body; superadmin sends the audit comment) — so that route parses the body defensively rather than via `zValidator('json')`. Reports keep the no-comment soft delete for now.
...
- **Domain modules:** `auth/`, `users/`, `customers/`, `contacts/`, `reports/`, `upload/`, `cms/`. `contacts/` owns `customer_contacts` (a client's people). The contact **entity** is a top-level resource — single-record ops at `/contacts` (`POST`, `GET/PATCH/DELETE /contacts/:id`, addressed by the contact's own uuid, `customerId` in the create body). The **collection** is parent-scoped: `GET /customers/:id/contacts` (on the customers controller, delegating to the contacts service). The customers module also delegates nested compose + inline create/update to the contacts service (`replaceContactsForCustomer`).
...
- **Per-module folders (create only what a module needs):**
  ...
  - `templates/*.template.ts` — static markup assets that a helper renders (e.g. the report-email HTML in `reports/templates/report-email.template.ts`). Markup lives here, never inline in a renderer. **Name these `*.template.ts`, never `*.html.ts`** — wrangler's default `.html` module rule matches the import specifier and loads the file as a Text asset (string default export), so the named markup import fails at `wrangler dev` boot (vitest doesn't apply that rule, so tests still pass — it only bites `pnpm dev`).
...
- One controller per resource under `src/modules/<resource>/controllers/*.controller.ts` (`auth`, `users`, `customers`, `contacts` — entity at `/contacts/:id`, collection at `/customers/:id/contacts` — `reports`, `upload`, `cms` — which also owns the unauthenticated `public-cms` controller). **Controllers stay thin** — validate → look up auth context → call a service → respond. Business rules/orchestration live in `services/`; queries in `repository/`.
...
- **Response envelope (migrating):** `customers` now speaks the superadmin standard — list → `PagedResponse` `{ items, total, page, limit }`, single/create/update → the **bare entity** (no `{ customer }` wrapper); `?page&limit` opt into paging, absent → full active list. `users`/`reports` still use the legacy `{ users }` / `{ report }` wrappers and will migrate to the same shape as each is wired to the superadmin. The legacy main frontend adapts customers at its http-service edge (`frontend/src/http/customers.service.ts` re-wraps into its own `{ customers }`/`{ customer }` DTOs).
...
- **Schema** is defined per-module in `models/*.model.ts` and re-exported from the barrel `src/modules/database/schema.ts` (which also holds every `relations()`). Tables: `users` (incl. `delete_comment`, `deleted_by` self-FK), `customers` (incl. `timezone`, CRM `status`/`source`/`tags[]`/`blacklist_reason`/`next_follow_up_at` + `delete_comment`/`deleted_by` audit), `customerContacts` (1:N people), `customerFiscal` (1:1 CFDI block), `reports`, `reportDetails`, `reportCounters`, `reportEmails`, `cmsDocuments`, `cmsClients`. Use Drizzle's column helpers; pull types via `typeof table.$inferSelect` / `$inferInsert` (aliased in each module's `types/*.types.ts`).
...
## Email + PDF
- Outbound only. The PDF is generated server-side via `pdf-lib`: the generic drawing toolkit lives in `modules/pdf/` (`services/pdf.service.ts` + `constants/pdf-layout.ts` + `types/pdf.types.ts`), and the report **document layout** that composes it is `modules/reports/helpers/report-pdf.helpers.ts`. The email HTML markup lives in `modules/reports/templates/report-email.template.ts`, filled by the renderer `modules/reports/helpers/report-email.helpers.ts`, then dispatched through `modules/reports/services/report-email.service.ts` (`dispatchReportEmail`), which calls the generic transport `modules/email/services/email.service.ts` (`sendEmail`, provider-swappable Resend wrapper).
