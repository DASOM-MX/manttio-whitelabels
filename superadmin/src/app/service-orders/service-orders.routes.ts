 ...
 *
 *  `VisitsState` rides along for the order view's "Programar visita": the visit
 *  dialog dispatches into it, route providers don't reach across siblings, and
 *  NGXS completes an unhandled dispatch without error — so without it the
 *  create would toast success having called nothing (quotations sets the
 *  precedent of providing a sibling module's states). */
export default [
  {
    path: '',
    providers: [provideStates([ServiceOrdersState, VisitsState])],
    children: [
