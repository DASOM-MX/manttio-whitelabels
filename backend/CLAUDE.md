...
- **`owner` role (whitelabel fork):** the tenant principal. Provisioning-time only — `users/enums` splits `ROLES` (valid everywhere) from `GRANTABLE_ROLES` (what POST/PATCH `/users` accept: admin/technician), and mutations targeting an owner row throw `CannotModifyOwnerError` → `403 cannot_modify_owner` (no in-tenant edit/delete/password-reset of the owner). Today `owner` gates alongside `admin` on `/cms/*` only; the full owner-vs-admin surface split lands with plan 14.
