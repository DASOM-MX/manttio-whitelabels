...
- **Rendering: charts via `chart.js` + PrimeNG `p-chart`** (new dependency, accepted). Brand-token colors, dark-mode aware. ~~**Revised 2026-07-20 (owner): pie charts, not grouped bars** — each pie shows the current-period channel mix (zero channels dropped), sliced through a single-hue primary-scale ramp with a right-side legend; the previous-month comparison lives in the KPI deltas, not the charts.~~ **Superseded 2026-07-22 (owner, "CRM cockpit" executive redesign — branch `feature/fullstack-crm-dashboard-refresh`): the pies are retired.** The page now reads: (1) **KPI strip** — Leads / Nuevos activos / Conversión (share of the period's intake already active, delta in pp) / Seguimientos vencidos, on the 01 stat-card idiom (micro-label + `font-data` value + signed colored-text delta + trailing `.icon-chip`); (2) **six-month trend line** (`GET /customers/stats/trend`, zero-filled UTC month buckets, same snapshot semantics) — hero Leads line with the sanctioned single-hue `primary-400` area fill, Nuevos activos as the dark-neutral second line, dot-chip legend in the card header; (3) **channel mix as proportional single-hue bars** (`primary-400`, width relative to the top channel, "N leads · M activos" split line) — derived from the intake rows, no new endpoint; (4) **follow-up agenda** (`GET /customers/follow-ups` — items soonest-first + whole-scope `overdueCount`/`scheduledCount`; excludes blacklisted; date pill red overdue / amber "Hoy" / neutral upcoming); (5) the two feeds on the CP-3 `#loadingbody` skeleton idiom. Both new endpoints share the module gate (owner/admin/office). **Layout turn (owner, same day):** row order is trend + **clientes recientes on its rail** (compact 2-col: name with contact/source sub-line, alta) → channels + **activity beside them with the richer columns** (new Estatus `p-tag` — `customerStatus` joined into `GET /customers/interactions/recent` — plus author under the date) → **follow-up agenda full-width** at the bottom (gains an Origen column — `source` joined into `/customers/follow-ups`, NULL bucketed as `other`). The truncated Registro cell serves the **full record via stock `pTooltip` on hover** (owner, same day — first tooltip in the app; preset widens `tooltip.root.maxWidth` to `24rem` so bodies read as sentences, not ribbons).
