...
(Serializado / A granel), `material-unit-status-labels.const.ts` +
...
optional). **Tracking is immutable after create** (01 §2 — backend rejects once
...
- **Stock by location** (`GET /materials/:id/stock`): table warehouse → node → quantity;
  ...
  ("12 en stock · 3 consumidas · 1 perdida").
